...
### 4. MultiAgent 委托
- 固定子 Agent：通过 `SUBAGENT.yaml` 预定义（角色、工具、权限）
- 动态子 Agent：LLM 在运行时通过 `subagent_create` 按需创建
...
主 Agent 可以把任务委托给子 Agent，每个子 Agent 独立运行自己的 ReAct 循环：

- **固定子 Agent**：通过 `SUBAGENT.yaml` 预定义角色、工具白名单和 token 限制，启动时加载。
- **动态子 Agent**：LLM 运行时通过 `subagent_create` 按需创建，自行指定角色和工具。
- **前台执行**（默认）：主 Agent 阻塞等待，支持超时和取消信号透传。
- **后台执行**：子 Agent 异步运行，完成后将结果写入 `.subagent/{id}.md`，主 Agent 按需读取。
- **单层约束**：子 Agent 不能再创建子 Agent，保持架构扁平、便于调试。

### 涌现行为

后台子 Agent 启动后，框架不会主动提示"去轮询结果"，但 LLM 自发产生了等待-检查策略：
...
用户只与主 Agent 对话，子 Agent 对用户透明。按 Esc 可取消所有 Agent（含后台）。因此，未来的"规划模式"应作为主 Agent 的内部状态切换，而非独立的子 Agent。
