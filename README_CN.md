...
- **固定子 Agent**：在 `SUBAGENT.yaml` 中声明（角色、允许的工具、token 限制），启动时注册。
- **动态子 Agent**：LLM 在运行时通过 `subagent_create` 按需创建，自行指定角色和工具。
...
**为什么用前缀而非后缀？** 因为框架需要在 LLM 采样阶段对工具进行分组 mask（允许/禁用）。统一前缀后，一个 `file_*` 模式即可选中整个类别，无需逐个枚举。这在三个场景中尤为重要：

1. **子 Agent `allowed_tools`** — 只读探索型子 Agent 配置 `["file_read", "bash"]`，前缀分组让白名单一目了然、不易出错。
2. **采样阶段 mask** — 评估模型时，可能需要一条规则禁用所有写入工具或所有子 Agent 工具。基于前缀的过滤让这变成简单的 glob 匹配。
3. **未来工具策略** — 随着工具集增长，类别前缀让命名空间保持扁平且自描述。新增的 `file_search` 自动归入 `file_*` 组，零配置。
