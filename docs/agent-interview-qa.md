...
**Skills 是给 LLM 的结构化说明文档，用于封装可复用的工作流，通过渐进式加载高效交付给 Agent。**

### 1. 给 LLM 的说明文档

核心是一份 Markdown 文档（SKILL.md），专门写给 LLM 看的 prompt，告诉 Agent 什么场景下该用、具体怎么一步步操作、有哪些约束。比如 `claude-api` skill 就是一份"如何用 Claude API 构建应用"的操作手册，Agent 读了就"会"选择合适的 SDK、编写流式请求、处理 Tool Use。

### 2. 封装可复用的工作流

Skills 把复杂任务封装成多步骤的 SOP。比如 `claude-api` 定义了完整流程：检测项目语言 -> 选择合适的 Surface（单次调用 / Workflow / Agent SDK）-> 加载对应语言的参考文档 -> 按模式实现，还涵盖 streaming、batch、tool use 等高级模式。不同用户、不同项目，触发同一个 Skill 就能按相同流程执行。
...
| 层级 | 加载内容 | 时机 | Token 开销 |
|------|---------|------|-----------|
| L1 元数据 | name + description | 启动时写入 system prompt | ~100/skill |
| L2 指令体 | SKILL.md 完整内容 | Agent 调用 `skill_get` 按需加载 | < 5000 |
| L3 引用资源 | scripts/、references/ | 执行过程中按需读取 | 按需 |

### 4. Skills 与工具调用的关系

Skills 提供知识和流程，Tools 提供执行能力，Agent 根据 Skill 的指引决定调用哪些 Tools。

- Skill 指导 Agent 调用哪些工具、按什么顺序调用。比如 `claude-api` skill 告诉 Agent：先用 Read 检测项目语言 -> 再用 `skill_get` 加载对应语言参考文档 -> 用 Write 生成代码 -> 用 Bash 安装 SDK 并运行测试。
- 没有 Skill，Agent 也能调用工具，但缺少领域最佳实践；没有 Tool，Skill 的指令无法执行。
- Skill 本身也通过工具交付——Agent 通过 `skill_get` 工具按需加载 Skill 内容。
...
- **通信范式**：RPC（Remote Procedure Call），像调用本地函数一样调用远程服务
- **消息协议**：定义消息格式和语义，如 JSON-RPC 2.0、REST API
- **传输层**：负责把消息送到对方，如 HTTP、Stdio 管道

### 项目中的远程调用

**1. LLM API 调用（REST over HTTPS）**

SDK 封装了 HTTP 请求，本质是 RPC——`client.messages.create(model=..., messages=...)` 看起来像调本地方法，实际发了一个 HTTPS 请求到远端。

| 调用方 | 库 | 连接目标 |
|--------|-----|---------|
| `AnthropicClient` | `anthropic` (AsyncAnthropic) | Anthropic API、MiniMax 网关 |
| `OpenAIClient` | `openai` (AsyncOpenAI) | OpenAI API、MiniMax 网关、SiliconFlow |
| `BedrockClient` | `anthropic` (AsyncAnthropicBedrock) | AWS Bedrock（HTTPS + SigV4 签名认证） |

**2. MCP 工具调用（JSON-RPC 2.0）**

`session.call_tool(name, arguments)` 是 JSON-RPC 2.0 调用，消息可以跑在三种传输上：

| 传输方式 | 通信模式 | 适用场景 |
|---------|---------|---------|
| Stdio | 双向管道 | 本地 MCP Server，无网络开销 |
| SSE | 服务端->客户端单向推送 | 远程 MCP Server，流式返回 |
| Streamable HTTP | 双向流式 | 远程 MCP Server（MCP 推荐） |

**3. Skills 层调用**

Skills 的远程调用发生在两个层面：

- **指令层面**：Skill 的 Markdown 内容引导 Agent 通过工具发起远程请求。比如 `mcp-builder` skill 指示 Agent 用 WebFetch 抓取远程文档，`claude-api` skill 示例中用 curl 直接调用 Anthropic API。
- **脚本层面**：`scripts/` 中的代码直接发起远程连接。比如 `mcp-builder/scripts/connections.py` 实现 MCP 远程连接，`evaluation.py` 调用 Anthropic API 做评估。

### HTTP 上的不同实现

除 Stdio 外，项目中的远程调用都基于 HTTP，按角色区分：

| 实现 | 角色 | 项目中的使用 |
|------|------|-------------|
| REST API | API 设计风格 | LLM API 调用，请求-响应，一次调用一次完整返回 |
| SSE | 服务端单向推送机制 | MCP 远程连接，server 持续推送工具执行结果 |
| Streamable HTTP | 双向流式传输机制 | MCP 远程连接（推荐），client 和 server 都可流式发送 |
| curl / WebFetch | HTTP 客户端工具 | Skills 中引导 Agent 抓取远程文档或调用 API |

分支不是协议层面的差异，是同一个 HTTP 上不同通信模式的选择，由实时性需求决定。

### 补充：WebSocket

WebSocket 和 HTTP 同处传输层，建立连接后支持全双工通信（任意一方随时发送消息），延迟最低。MCP 协议早期使用过 WebSocket，后来替换为 Streamable HTTP，原因是 WebSocket 对企业网络基础设施（代理、负载均衡、防火墙）的兼容性较差，而 Streamable HTTP 基于标准 HTTP 语义，无需额外的基础设施支持。

### 补充：ACP（Agent Client Protocol）

ACP 是编辑器/IDE 与 Coding Agent 之间的标准通信协议，官网 agentclientprotocol.com。类似 LSP 的思路——Agent 实现一次 ACP，就能接入所有支持 ACP 的编辑器（Zed、Emacs、Neovim 等）。

兄弟项目 Mini-Agent 的 `mini_agent/acp/` 模块实现了 ACP，通过 Stdio 传输让 Zed 编辑器调用 Mini-Agent。协议复用了 MCP 的 JSON 消息格式，在其基础上扩展了 diff 展示、终端管理等编辑器特有的交互能力。